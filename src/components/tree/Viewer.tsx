...
import { reconcileEdges } from "@/lib/reconcileEdges"; 
...
export function SkillTreeViewer({
  ...
  React.useEffect(() => {
    setEdges((prev) => reconcileEdges(nodes, prev));
  }, [nodes]);
  ...
  const onFork = React.useCallback(async () => {
    ...
    // Always fork a normalized snapshot so the new tree is valid
    const forkNodes = JSON.parse(JSON.stringify(nodes));
    const forkEdges = reconcileEdges(forkNodes, edges);
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-[#f4f4f3]">
      ...
      <InstallSkillsDialog open={installOpen} onOpenChange={setInstallOpen} nodes={nodes} />
